...
def collect_mcu(
    ...
    """Parse SVD, dedup register templates for this MCU, return MCU data."""
    registry = RegisterRegistry()
    ...
    for p in peripherals:
        ...
        for reg in p.registers:
            ...
            sig = reg.signature
            label = f'{mcu}::{p.name}'
            registry.get_or_create(ptype, reg.name.lower(), sig, reg, label)
...
def register_template_lines(templates: list[RegisterTemplate]) -> list[str]:
    """Emit `regs` namespace body lines for the given register templates."""
...
def generate_peripheral_files(
    ...
    """Generate the per-MCU <family>/<mcu>/<ptype>.hpp header.

    Register templates and peripheral groups are both inlined here; nothing
    is shared between MCUs.
    """
    templates = sorted(
        (t for t in sig_to_template.values() if t.periph_type == ptype),
        key=lambda t: (t.reg_name, t.version),
    )
    reg_lines = register_template_lines(templates)
...
def generate_addresses_header(
    ...
    """Generate <mcu>/addresses.hpp with all base addresses."""
    lines = []
...
def generate_aggregate(
    ...
    """Generate the <family>/<mcu>.hpp aggregate header."""
    lines = []
...
def emit_mcu_files(
    ...
    """Emit <family>/<mcu>.hpp plus <family>/<mcu>/<ptype>.hpp + addresses."""
    mcu = mcu_data.mcu
    ...
    for ptype, p_list in sorted(mcu_data.type_peripherals.items()):
        content = generate_peripheral_files(
            ...
            mcu_data.registry.sig_to_template,
            mcu_data.shared_ns,
    ...
    (mcu_dir / 'addresses.hpp').write_text(addresses)
    ...
    (family_dir / f'{mcu}.hpp').write_text(aggregate)
...
def main():
    ...
    stale_common = output_base / 'common'
    for sub in ('registers', 'peripherals'):
        if (stale_common / sub).exists():
            shutil.rmtree(stale_common / sub)

    # Phase 1: parse all MCUs, dedup register templates per MCU
    all_mcu_data: list[MCUData] = []
    for svd_path in args.svd_files:
        mcu_data = collect_mcu(svd_path, verbose=args.verbose)
        all_mcu_data.append(mcu_data)

    # Phase 2: emit per-MCU peripheral, address, and aggregate files
    for mcu_data in all_mcu_data:
        emit_mcu_files(mcu_data, output_base, verbose=args.verbose)

    # Phase 3: mcu -> family manifest, read by meson.build at configure time
    manifest_lines = [
    ...
    if args.stats:
        ...
        print(f"Register dedup ratio:    "
              f"{(1 - total_tmpl / total_regs) * 100:.1f}% "
